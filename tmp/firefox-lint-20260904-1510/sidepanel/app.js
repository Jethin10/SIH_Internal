...
function renderReceipts(receipts) {
  ...
  $("receipts").innerHTML = state.receipts.length
    ? state.receipts.slice(0, 20).map((item) => `<div class="log-row"><b>${escapeHTML(item.action)}</b> <code>${escapeHTML(item.localDecision)}</code><small>${escapeHTML(item.origin || "local")} · ${escapeHTML(item.risk || "low")} risk · ${escapeHTML(new Date(item.at).toLocaleTimeString())}${item.usedPrivateToken ? " · private alias resolved locally" : ""}</small></div>`).join("")
