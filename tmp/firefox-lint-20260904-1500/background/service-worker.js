...
if (chrome.sidebarAction?.toggle && chrome.action?.onClicked) {
...
async function runVisualOperation(type, payload) {
  const usesOffscreen = await ensureOffscreenDocument();
  if (usesOffscreen) return chrome.runtime.sendMessage({ target: "offscreen", type, ...payload });
  ...
  throw new Error("Local visual processor is unavailable in this browser context");
}
