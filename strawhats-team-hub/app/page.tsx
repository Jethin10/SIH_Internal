...
const areas = [
  ...
  },
  {
    ...
    build: ['Verify the implemented planner server and its authenticated upstream mode', 'Configure and run the real-model verification command on a synthetic task', 'Maintain rejection tests for malformed responses, unknown actions and invented targets', 'Record full task time and provider round-trip time with their measurement limits'],
  ...
  },
  {
    ...
    build: ['Run the evaluation and browser journeys on the presentation laptop', 'Collect an independent held-out PII and visual-redaction dataset alongside the synthetic challenge corpus', 'Record CPU and whole-browser memory use on at least two machines', 'Produce one judge-ready results table with method and limitations'],
  ...
  },
  {
    ...
    build: ['Build and verify Chrome, Firefox and source archives with the portable Node release tool', 'Install the package on a second computer and run the full demo', 'Verify the Windows/macOS/Linux CI matrix and the Firefox runtime evidence', 'Rehearse five times and record one clean backup video'],
