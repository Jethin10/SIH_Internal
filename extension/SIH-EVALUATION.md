...
The contextual development challenge (`npm run test:privacy`) contains 47 hand-authored English/Hindi, formatting, OCR-error-text, and negative cases. It scores 42 expected entity occurrences with zero false positives or false negatives (100% precision and recall on this development corpus). The independent command (`npm run test:privacy:independent`) uses a checked-in snapshot of 99 mapped labels and 20 negative documents from the Apache-2.0 Gretel test split at revision `7b844d16738527a04264f50214cb426a4cea0897`. It currently reports 92/99 (92.9%) exact type-and-value recall and a 100% clean-negative rate. The source-development window (rows 0–499) is excluded from the frozen evaluation window (rows 1000–1999). These scores are kept separate from the generated 1,254-case regression score. Representative real-world data, OCR engine evaluation across varied images, and whole-browser CPU/memory measurements on a second machine remain future evidence work.
